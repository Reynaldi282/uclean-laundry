...

{
  ...
  public function store(AddOrderRequest $request)
  {
    try {
      ...
      $order->tgl_transaksi   = now()->format('d/m/Y H:i');
      ...
      $tanggal_selesai    = date('d/m/Y H:i', strtotime('+' . $order->hari . ' days', strtotime($order->tgl_transaksi)));
      $judul = nama_laundry(1);
      $alamat = alamat_laundry($order->user_id);
      $noTelp_laundry = noTelp_laundry($order->user_id);
      $jenisPakaian = get_harga_by_id($order->harga_id);
      $hitungRupiah = Rupiah::getRupiah($hitung);
      if ($request->disc != NULL) {
        $discRupiah = Rupiah::getRupiah($disc);
      } else $discRupiah = 0;
      $harga_akhir = Rupiah::getRupiah($order->harga_akhir);

      if ($order) {
        //Notification WhatsApp
        if (setNotificationWhatsappOrderSelesai(1) == 1 && getTokenWhatsapp() != null) {
          $waCustomer = User::where('id', $order->customer_id)->first()->no_telp; // get nomor whatsapp customer
          $nameCustomer = User::where('id', $order->customer_id)->first()->name; // get name customer
          notificationWhatsapp(
            getTokenWhatsapp(), // Token
            $waCustomer, // nomor whatsapp
            "FAKTUR ELEKTRONIK TRANSAKSI REGULER
$judul
$alamat
$noTelp_laundry
...
Terima : $order->tgl_transaksi
Selesai : $tanggal_selesai

======================
Detail pesanan:
Layanan:
$jenisPakaian->jenis.' '.$order->kg
'@'.$jenisPakaian->harga.', Total '.$hitungRupiah

==============
Detail biaya : 
Total tagihan : $hitungRupiah
Diskon : ($order->disc) $discRupiah
Grand total : $harga_akhir

Pembayaran: 
💵 $order->jenis_pembayaran $harga_akhir
