...
class SettingsController extends Controller
{
  ...
  public function proses_set_page(Request $request, $id)
  {
    ...
    if ($request->hasFile('img_hero')) {
      $fileNameExt = $request->file('img_hero')->getClientOriginalName();
      $fileName = pathinfo($fileNameExt, PATHINFO_FILENAME);

      $fileExt = $request->file('img_hero')->getClientOriginalExtension();
      $fileNameToStore = $fileName . '_' . time() . '.' . $fileExt;
      $tujuan_upload = 'frontend/img/logo';
      $pathToStore = $request->file('img_hero')->move($tujuan_upload, $fileNameToStore);
    }

    $setpage = PageSettings::find($id);
    $setpage->judul     = $request->judul;
    if($request->hasFile('img_hero')){
      $setpage->img_hero = $pathToStore;
  }
